...
void sendSerialTelemetry() {
  ...
  case 'B': // Send roll, pitch and yaw PID values
    ...
    SerPri(KI_QUAD_YAW);
    comma();
    SerPri(STABLE_MODE_KP_RATE_YAW);
    comma();
    SerPri(STABLE_MODE_KP_RATE);    // NOT USED NOW
    comma();
    SerPrln(MAGNETOMETER);
    queryType = 'X';
    break;
  case 'D': // Send GPS PID
    SerPri(KP_GPS_ROLL);
    ...
    SerPri(KD_GPS_PITCH);
    comma();
    SerPri(GPS_MAX_ANGLE);
    comma();
    SerPrln(GEOG_CORRECTION_FACTOR);
    queryType = 'X';
    break;
  ...
  case 'S': // Send all flight data
    ...
    SerPri(read_adc(5));
     ...
     comma();
     SerPri(AP_Compass.heading_x);
     comma();
     SerPri(AP_Compass.heading_y);
     comma();
