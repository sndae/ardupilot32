...
//  HardwareSPI spi2(2);  // creo la classe SPI2
  #define SPIADC                   SPI1
  #define SPIADC_CLK               RCC_APB2Periph_SPI1
  #define SPIADC_GPIO              GPIOA
  #define SPIADC_GPIO_CLK          RCC_APB2Periph_GPIOA  
...
unsigned char AP_ADC_ADS7844::ADC_SPI_transfer(unsigned char data)
{
	unsigned char ret_byte;
	//ret_byte=spi2.send(data);  // la vecchia funzione invia un byte comando e riceve un byte risposta.
	return ret_byte;	
}
...
void AP_ADC_ADS7844::Init(void)
{
 ...
 digitalWrite(31,HIGH);
 Serial1.printf("CIAO");
// spi2.begin(SPI_1_125MHZ   ,MSBFIRST ,0);	// inizializzo la i2c
