...
// ClearAll - clears all data from static members
void APM_PerfMon::DisplayResults()
//void APM_PerfMon::DisplayResults(HardwareSerial* aSerial)
{
	...
	SerialUSB.print("PerfMon start:");
	SerialUSB.print(allStartTime/1000);
	SerialUSB.print(" (mils)   end:");
	SerialUSB.print(allEndTime/1000);
	SerialUSB.print(" (mils)  elapsed:");
	SerialUSB.print(totalTime/1000);
	SerialUSB.print(" (mils)");
	SerialUSB.println();
	SerialUSB.println("PerfMon:           \tcpu%\tmils\t#called\tHz");
	for( i=0; i<nextFuncNum; i++ )
	{
	    j=order[i];
	    sumOfTime += time[j];
	    hz =(float) numCalls[j]/((float)totalTime/1000000.0);
		pct = ((float)time[j] / (float)totalTime) * 100.0;
		padding = PERFMON_FUNCTION_NAME_LENGTH - strLen(functionNames[j]);
		
		SerialUSB.print(functionNames[j]);
		for(k=0;k<padding;k++)
		    SerialUSB.print(" ");
		SerialUSB.print("\t");
		SerialUSB.print(pct);
		SerialUSB.print("%\t");
		SerialUSB.print(time[j]/1000);
		SerialUSB.print("\t");
		SerialUSB.print(numCalls[j]);
		SerialUSB.print("\t");		
		SerialUSB.print(hz);
		SerialUSB.print("hz");
		SerialUSB.println();
	...
	SerialUSB.print("unexplained        \t");
	SerialUSB.print(pct);
	SerialUSB.print("%\t");
	SerialUSB.print(unExplainedTime/1000);
	SerialUSB.println();
