...
void RunCLI () {
  ...
  //SerBeg(SerBau);   
  SerPrln();
  SerPrln("Welcome to ArduCopter CLI (Arm Fox Branch by Redfox74"); 
  SerPrln("Check the official thread for Project Update");
  SerPrln("http://www.virtualrobotix.com/forum/topics/multipilot-20-sample-will-be");  
  SerPri("Firmware: ");
  SerPrln(VER);
  SerPrln();
  ...
  while (exit==0) { 
    ...
    if (SerAva()) {
      ...
      case 'z':
        ...
        break;        
      case 'x':
        SerPrln("Exit From CLI");
        exit=1; // Exit from CLI
      }
      //SerFlu();
    }
