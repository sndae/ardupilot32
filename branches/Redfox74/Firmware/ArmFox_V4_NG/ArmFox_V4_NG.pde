...
/* ********************************************************************** */
/* Hardware : ArduPilot 32 + Sensor Shield (Production versions)        */
...
#ifdef SerXbee
  //SerialUSB.begin(SerBau);
  SerialUSB.print("ArduCopter v");
  SerialUSB.println(VER);
  SerialUSB.println("Serial data on Telemetry port");
  SerialUSB.println("No commands or output on this serial, check your Arducopter.pde if needed to change.");
  SerialUSB.println();
  SerialUSB.println("General info:");
  if(!SW_DIP1) SerialUSB.println("Flight mode: + ");
  if(SW_DIP1) SerialUSB.println("Flight mode: x ");
#endif 
